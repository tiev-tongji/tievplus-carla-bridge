...
int NamedMapArray::encode(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
...
int NamedMapArray::decode(const void *buf, int offset, int maxlen)
{
    int pos = 0, thislen;
...
int NamedMapArray::getEncodedSize() const
{
...
int NamedMapArray::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    int pos = 0, tlen;
    ...
    if(this->width > 0) {
        for (int a0 = 0; a0 < this->height; a0++) {
            tlen = __byte_encode_array(buf, offset + pos, maxlen - pos, &this->cells[a0][0], this->width);
            if(tlen < 0) return tlen; else pos += tlen;
        }
    }
...
int NamedMapArray::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    int pos = 0, tlen;
    ...
    for (int a0 = 0; a0 < this->height; a0++) {
        if(this->width) {
            ...
            tlen = __byte_decode_array(buf, offset + pos, maxlen - pos, &this->cells[a0][0], this->width);
            if(tlen < 0) return tlen; else pos += tlen;
        }
    }
...
int NamedMapArray::_getEncodedSizeNoHash() const
{
    int enc_size = 0;
...
uint64_t NamedMapArray::_computeHash(const __lcm_hash_ptr *)
{
    uint64_t hash = 0x4c98461865fa6b96LL;
